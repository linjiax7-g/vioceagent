...
async def web_search(
    ...
    # NOTE: Removed fallback to general search to ensure we ONLY use shopping endpoint
    # This ensures all web results are from shopping sources only
    if search_type == "shopping" and not results_out:
        print("[WEB SEARCH] Shopping endpoint returned 0 results. Keeping empty results (no fallback to general search).")
