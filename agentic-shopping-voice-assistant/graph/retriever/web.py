...
def retrieve_from_web(
    ...
    filtered_docs = _filter_and_enhance_results(docs, query, filters)
...
def _filter_and_enhance_results(docs: List[Dict], original_query: str, filters: Dict = None) -> List[Dict]:
    """
    ...
    Args:
        ...
        filters: Price and other filters to apply
            ...
            if "min_price" in filters and price:
                try:
                    if price < float(filters["min_price"]):
                        logger.debug(f"[WEB] Filtering out product below min_price: ${price} < ${filters['min_price']}")
                        continue
                except (ValueError, TypeError):
                    pass
            
            if "max_price" in filters and price:
                try:
                    if price > float(filters["max_price"]):
                        logger.debug(f"[WEB] Filtering out product above max_price: ${price} > ${filters['max_price']}")
                        continue
                except (ValueError, TypeError):
                    pass
    ...
    if filtered:
        from graph.retriever.rag import _validate_results_with_groq
        filtered = _validate_results_with_groq(original_query, filtered)
